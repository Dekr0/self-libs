...
class ClassifyByTarget(C274):
    ...
    def initTF(self):
        # init or reset vars for confusions matrix
        self.TP = 0
        self.FP = 0
        self.TN = 0
        self.FN = 0
...
class TrainingSet(C274):
    ...
    def inspect_comment(self, line):
        if len(line) > 1 and line[1] != ' ':      # Might be variable
            ...
            self.set_env_variable(v[0][1:], v[1])  # v[0][1:] - remove %
        return
    ...
    def process_input_stream(self, inFile, run=None):
        ...
        cFlag = True
        while cFlag:
            # read line by line (sentence by sentence) in a file using while loop
