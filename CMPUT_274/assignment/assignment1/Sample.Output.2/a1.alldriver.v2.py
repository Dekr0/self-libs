...
def main():
    ...
    tset.preprocess()
    run1.target_top_n(tset, num=4, label=plabel)  # Call to new Task II method
    # FIXME Flaw in ooclassifier.py means that we need to drop
    #   the most frequent target word if it is also the positive label
    # FIXME Alternate way of fixing the problem with the class label being
    #   confused with a (feature) word, as a result of preprocessing.
    #   The "real" fix (after Assignment #1 is due) is to change
    # File:  ooclassifier.py, change from:
    #       246                 tlabel="read", inclLabel=True
    # to:
    #       246                 tlabel="read", inclLabel=False
    # That's the one line fix (above) that I am avoiding, with this hack.
    tw = run1.get_target_words()
    if plabel[1:] in tw:     # Remove (once) a word, if also label
        tw.remove(plabel[1:])
